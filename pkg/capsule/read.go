...
func readToken(r *reader) (Capsule, error) {
	token := r.next()
	if token == nil {
	...
	if match, _ := regexp.MatchString(`^-?[0-9]+$`, *token); match {
		...
		return Symbol(strings.ToUpper(*token)), nil
	}
}

func readList(r *reader, start, end string) (Capsule, error) {
	token := r.next()
	if token == nil {
...
func readMap(r *reader) (Capsule, error) {
	list, err := readList(r, "{", "}")
	if err != nil {
		return nil, err
	}
	return listToMap(list.(Pallete))
}

func read(r *reader) (Capsule, error) {
	token := r.peek()
	if token == nil {
		log.Fatal("tokens underflow")
	}
	...
	case "'":
		...
		return Pallete{Symbol("QUOTE"), q}, nil
	case "`":
		r.next()
		f, err := read(r)
		if err != nil {
		...
		return Pallete{Symbol("QUASIQUOTE"), f}, nil
	case "~":
		r.next()
		f, err := read(r)
		if err != nil {
		...
		return Pallete{Symbol("UNQUOTE"), f}, nil
	case "~@":
		r.next()
		f, err := read(r)
		if err != nil {
		...
		return Pallete{Symbol("SPLICE-UNQUOTE"), f}, nil
	case ")", "}", "]":
...
func Read(s string) (Capsule, error) {
	tokens := tokenize(s)
	if len(tokens) == 0 {
		return nil, nil
	}
