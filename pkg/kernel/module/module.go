...
func Insert(path string, options string) error {
	...
	if _, _, errno := syscall.Syscall(syscall.SYS_INIT_MODULE, uintptr(unsafe.Pointer(&data[0])), uintptr(len(data)), 0); errno != 0 {
		return errno
	}
	return nil
}
...
func Load(name string, searchPath string, loaded map[string]bool) error {
	path, err := Search(name, searchPath)
	if err != nil {
		return err
	}
...
func Cache(searchPath string) error {
	var is []Info
	if err := filepath.Walk(searchPath, func(path string, info fs.FileInfo, err error) error {
	...
	return os.WriteFile(filepath.Join(searchPath, "cache.json"), out, 0644)
}
