...
void Sysroot::load_deployments() {
    ...
    std::unique_ptr<OstreeDeployment, decltype(&g_object_unref)> booted_deployment(
            ostree_sysroot_get_booted_deployment(backend), g_object_unref);

    if (booted_deployment == nullptr) {
        throw std::runtime_error("no booted deployment found");
    }

    std::unique_ptr<GPtrArray, void (*)(GPtrArray *)> deployment_list(ostree_sysroot_get_deployments(backend),
                                                                      +[](GPtrArray *array) -> void {
                                                                          g_ptr_array_free(array, true);
                                                                      });
    for (auto i = 0; i < deployment_list->len; i++) {
        deployments.emplace_back(reinterpret_cast<OstreeDeployment *>(deployment_list->pdata[i]), repo);

        if (ostree_deployment_equal(deployments.back().backend, booted_deployment.get())) {
...
void Sysroot::install(const std::vector<std::string> &refs) {
    ...
    for (auto const &ref: refs) {
        if (std::find_if(deployment.extensions.begin(), deployment.extensions.end(),
                         [&ref](const std::pair<std::string, std::string> &p) -> bool { return p.first == ref; }) ==
...
void Sysroot::uninstall(const std::vector<std::string> &refs) {
    auto deployment = get_active();
    for (auto const &ref: refs) {
        std::erase_if(deployment.extensions, [&ref](const std::pair<std::string, std::string> &p) -> bool {
            return p.first == ref;
        });
    }
...
Sysroot::pull(const Deployment &deployment, std::vector<std::string> &updated_revisions, bool dry_run, bool forced) {
    ...
    std::unique_ptr<OstreeAsyncProgress, decltype(&g_object_unref)> progress(
            ostree_async_progress_new_and_connect(progress_callback, this), g_object_unref);

    std::vector<std::string> revisions;
    auto merged_deployment = ostree_sysroot_get_merge_deployment(backend, OSNAME);
    ...
    if (!ostree_repo_pull(repo, OSNAME, (char **) (crefs.data()),
                          (dry_run ? OSTREE_REPO_PULL_FLAGS_COMMIT_ONLY : OSTREE_REPO_PULL_FLAGS_NONE), progress.get(),
                          nullptr, &error)) {
        throw Error(error);
    }
    if (progress) {
    ...
    for (const auto &refspec: refs) {
        auto [ref_changed, ref_revision, ref_changelog] = get_changelog("rlxos:"+refspec, revisions[i++]);
    ...
    std::string revision;
    std::unique_ptr<GKeyFile, decltype(&g_key_file_unref)> origin(nullptr, g_key_file_unref);
    ...
    for (int i = 1; i < refs.size(); i++) {
        ...
        if (!ostree_repo_write_directory_to_mtree(repo, commit.get(), mutableTree.get(), nullptr, nullptr, &error))
            throw Error(error);
    }
    GFile *res;
    if (!ostree_repo_write_mtree(repo, mutableTree.get(), &res, nullptr, &error))
        throw Error(error);
    std::unique_ptr<GFile, decltype(&g_object_unref)> root(res, g_object_unref);
    gchar *commit_checksum;
    std::unique_ptr<GVariantDict, decltype(&g_variant_dict_unref)> options(g_variant_dict_new(nullptr),
    ...
    for (int i = 0; i < deployment.extensions.size(); i++) {
        g_variant_dict_insert_value(options.get(), ("rlxos.revision." + deployment.extensions[i].first).c_str(),
                                    g_variant_new_string(updated_revisions[i + 1].c_str()));
    }

    if (!ostree_repo_write_commit(repo, nullptr, nullptr, nullptr, g_variant_dict_end(options.get()),
                                  OSTREE_REPO_FILE(res), &commit_checksum, nullptr, &error)) {
        throw Error(error);
    }
    ostree_repo_transaction_set_ref(repo, nullptr, "x86_64/os/local", commit_checksum);
    ...
    g_key_file_set_string_list(origin.get(), "rlxos", "extensions", ext_id.data(), ext_id.size());
    g_key_file_set_string(origin.get(), "rlxos", "channel", deployment.channel.c_str());

    std::unique_ptr<OstreeDeployment, decltype(&g_object_unref)> new_deployment(nullptr, g_object_unref);

    {
        OstreeDeployment *res;
        if (!ostree_sysroot_deploy_tree_with_options(backend, OSNAME, revision.c_str(), origin.get(),
        ...
        if (!ostree_sysroot_simple_write_deployment(backend, OSNAME, res, merged_deployment,
...
Sysroot::get_changelog(const std::string &refspec, const std::string &revision) {
    gchar *updated_revision = nullptr;
    GError *error = nullptr;
    if (!ostree_repo_resolve_rev(repo, refspec.c_str(), false, &updated_revision, &error)) {
        throw Error(error);
    }
    if (revision == updated_revision) { return {false, revision, ""}; }
    std::unique_ptr<GVariant, decltype(&g_variant_unref)> commit(nullptr, g_variant_unref);
    GVariant *result;
    if (!ostree_repo_load_variant(repo, OSTREE_OBJECT_TYPE_COMMIT, updated_revision, &result, &error)) {
        throw Error(error);
    }
    commit.reset(result);
    ...
    const gchar *body;
    guint64 timestamp;
    g_variant_get(commit.get(), "(a{sv}aya(say)&s&stayay)", nullptr, nullptr, nullptr, &subject, &body, &timestamp,
                  nullptr, nullptr);

    return {true, updated_revision,
            refspec + ":" + updated_revision + " " + std::to_string(timestamp) + " " + subject + " " + body};
}

Deployment Sysroot::get_active() const {
    auto deployment = std::find_if(deployments.begin(), deployments.end(), [](const Deployment &deployment) -> bool {
...
std::optional<UpdateInfo> Sysroot::apply_changes(const Deployment &deployment, bool dry_run, bool force) {
...
std::vector<std::string> Sysroot::get_available() const {
    IterUserData data{.refs = {}, .channel = get_active().channel,};
    g_autoptr(GHashTable) refs;
    GError *error = nullptr;
    if (!ostree_repo_remote_list_refs(repo, OSNAME, &refs, nullptr, &error)) {
        throw Error(error);
    }
    g_hash_table_foreach(refs, +[](gpointer k, gpointer v, gpointer user_data) {
        ...
        if (ref.contains("/extension/") && ref.ends_with(data->channel)) {
...
void progress_callback(OstreeAsyncProgress *progress, gpointer user_data) {
    ...
    ostree_async_progress_get(progress, "outstanding-fetches", "u", &outstanding_fetches,
                              "outstanding-metadata-fetches", "u", &outstanding_metadata_fetches, "outstanding-writes",
                              "u", &outstanding_writes, "caught-error", "b", &caught_error, "scanning", "u", &scanning,
                              "scanned-metadata", "u", &n_scanned_metadata, "fetched-delta-parts", "u",
                              &fetched_delta_parts, "total-delta-parts", "u", &total_delta_parts,
                              "fetched-delta-fallbacks", "u", &fetched_delta_part_fallbacks, "total-delta-fallbacks",
                              "u", &total_delta_part_fallbacks, "status", "s", &status, NULL);
    ...
    if (*status != '\0') {
        ...
        ostree_async_progress_get(progress, "bytes-transferred", "t", &bytes_transferred, "fetched", "u", &fetched,
                                  "metadata-fetched", "u", &metadata_fetched, "requested", "u", &requested,
                                  "start-time", "t", &start_time, "total-delta-part-size", "t", &total_delta_part_size,
                                  NULL);

        formatted_bytes_transferred = g_format_size_full(bytes_transferred, G_FORMAT_SIZE_DEFAULT);
        ...
        if (total_delta_parts > 0) {
            ...
            if (bytes_sec > 0) {
                ...
                g_autofree char *formatted_est_time_remaining = formatted_time_remaining_from_seconds(
                        est_time_remaining);
                /* No space between %s and remaining, since
                 * formatted_est_time_remaining has a trailing space */
                g_string_append_printf(buf, "Receiving delta parts: %u/%u %s/%s %s/s %sremaining", fetched_delta_parts,
                                       total_delta_parts, formatted_fetched, formatted_total, formatted_bytes_sec,
                                       formatted_est_time_remaining);
            } else {
                g_string_append_printf(buf, "Receiving delta parts: %u/%u %s/%s", fetched_delta_parts,
                                       total_delta_parts, formatted_fetched, formatted_total);
            }
        } else if (scanning || outstanding_metadata_fetches) {
            g_string_append_printf(buf, "Receiving metadata objects: %u/(estimating) %s/s %s", metadata_fetched,
                                   formatted_bytes_sec, formatted_bytes_transferred);
        } else {
            g_string_append_printf(buf, "Receiving objects: %u%% (%u/%u) %s/s %s",
                                   (guint) ((((double) fetched) / requested) * 100), fetched, requested,
                                   formatted_bytes_sec, formatted_bytes_transferred);
        }
