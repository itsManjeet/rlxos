...
static void outputmgrapplyortest(
...
static const Key keys[] = {
        ...
        {MODKEY | WLR_MODIFIER_SHIFT, XKB_KEY_less, tagmon,
                {.i = WLR_DIRECTION_LEFT}},
        {MODKEY | WLR_MODIFIER_SHIFT, XKB_KEY_greater, tagmon,
...
#ifdef XWAYLAND
    if (client_is_x11(c)) return c->surface.xwayland->surface;
#endif
    return c->surface.xdg->surface;
}

static inline int toplevel_from_wlr_surface(
        struct wlr_surface* s, Client** pc, LayerSurface** pl) {
    ...
    if ((layer_surface = wlr_layer_surface_v1_try_from_wlr_surface(
                 root_surface))) {
    ...
    while (xdg_surface) {
        ...
        case WLR_XDG_SURFACE_ROLE_POPUP:
            if (!xdg_surface->popup || !xdg_surface->popup->parent) return -1;
        ...
        case WLR_XDG_SURFACE_ROLE_TOPLEVEL:
            ...
            type = c->type;
            goto end;
        case WLR_XDG_SURFACE_ROLE_NONE: return -1;
        }
    }
...
static inline void client_activate_surface(
        struct wlr_surface* s, int activated) {
...
static inline uint32_t client_set_bounds(
        Client* c, int32_t width, int32_t height) {
#ifdef XWAYLAND
...
#ifdef XWAYLAND
    if (client_is_x11(c)) return c->surface.xwayland->class;
#endif
...
#ifdef XWAYLAND
    if (client_is_x11(c)) return c->surface.xwayland->title;
#endif
...
static inline int client_is_stopped(Client* c) {
    ...
    if (waitid(P_PID, pid, &in, WNOHANG | WCONTINUED | WSTOPPED | WNOWAIT) <
        ...
        if (in.si_code == CLD_STOPPED || in.si_code == CLD_TRAPPED) return 1;
        if (in.si_code == CLD_CONTINUED) return 0;
    }
...
#ifdef XWAYLAND
    if (client_is_x11(c)) return c->surface.xwayland->override_redirect;
#endif
    return 0;
}

static inline void client_notify_enter(
        struct wlr_surface* s, struct wlr_keyboard* kb) {
    if (kb)
...
static inline void client_set_border_color(
        Client* c, const float color[static 4]) {
    int i;
...
static inline uint32_t client_set_size(
        Client* c, uint32_t width, uint32_t height) {
#ifdef XWAYLAND
    if (client_is_x11(c)) {
...
static inline void client_set_tiled(Client* c, uint32_t edges) {
    ...
    if (wl_resource_get_version(c->surface.xdg->toplevel->resource) >=
        ...
        wlr_xdg_toplevel_set_maximized(
                c->surface.xdg->toplevel, edges != WLR_EDGE_NONE);
    }
}
...
void arrange(Monitor* m) {
    ...
    wlr_scene_node_set_enabled(
            &m->fullscreen_bg->node, (c = focustop(m)) && c->isfullscreen);
...
void arrangelayers(Monitor* m) {
    ...
    for (i = 0; i < (int)LENGTH(layers_above_shell); i++) {
        wl_list_for_each_reverse(l, &m->layers[layers_above_shell[i]], link) {
            ...
            exclusive_focus = l;
            client_notify_enter(
                    l->layer_surface->surface, wlr_seat_get_keyboard(seat));
            return;
        }
    }
}
...
void closemon(Monitor* m) {
    ...
    if (!nmons) {
        ...
        do selmon = wl_container_of(mons.next, selmon, link);
...
void commitlayersurfacenotify(struct wl_listener* listener, void* data) {
    ...
    if (l->layer_surface->initial_commit) {
        wlr_fractional_scale_v1_notify_scale(
                layer_surface->surface, l->mon->wlr_output->scale);
        wlr_surface_set_preferred_buffer_scale(layer_surface->surface,
                (int32_t)ceilf(l->mon->wlr_output->scale));
...
void commitnotify(struct wl_listener* listener, void* data) {
    ...
    if (c->surface.xdg->initial_commit) {
        ...
        wlr_fractional_scale_v1_notify_scale(
                client_surface(c), c->mon->wlr_output->scale);
        setmon(c, NULL, 0);
...
void resize(Client* c, struct wlr_box geo, int interact) {
    ...
    c->resize = client_set_size(
            c, c->geom.width - 2 * c->bw, c->geom.height - 2 * c->bw);
...
void setmon(Client* c, Monitor* m, uint32_t newtags) {
    ...
    if (m) {
        ...
        resize(c, c->geom, 0);
        c->tags =
                newtags ? newtags : m->tagset[m->seltags]; /* assign tags of
                                                              target monitor */
...
void setup(void) {
    ...
    struct sigaction sa = {handlesig, 0, SA_RESTART};
    ...
    locked_bg = wlr_scene_rect_create(layers[LyrBlock], sgeom.width,
            sgeom.height, (float[4]){0.1f, 0.1f, 0.1f, 1.0f});
    wlr_scene_node_set_enabled(&locked_bg->node, 0);
    ...
    LISTEN_STATIC(
            &virtual_pointer_mgr->events.new_virtual_pointer, virtualpointer);
...
void toggleview(const Arg* arg) {
    uint32_t newtagset;
    if (!(newtagset = selmon ? selmon->tagset[selmon->seltags] ^
                                       (arg->ui & TAGMASK)
                             : 0))
        return;
