...
export struct Recipe
{
                    ...
                    const std::filesystem::path& search_path = {})
    {
        ...
        {
            for (const auto& dep : config.node["build-depends"])
            {
        ...
        {
            for (const auto& dep : config.node["sources"])
    ...
    void update_from_data(const std::string& data, const std::string& filepath)
    {
        ...
        {
            for (const auto& dep : config.node["depends"])
            {
        ...
        {
            for (const auto& b : config.node["backup"])
    ...
    [[nodiscard]] std::string resolve(
        const std::string& value, const Configuration& global,
        const std::map<std::string, std::string>& extra = {}) const
    {
        std::map<std::string, std::string> variables = extra;
        if (global.node["variables"])
        {
            for (const auto& v : global.node["variables"])
            {
                variables[v.first.as<std::string>()] =
                    v.second.as<std::string>();
            }
        }

        if (this->config.node["variables"])
        {
            for (const auto& v : this->config.node["variables"])
            {
                variables[v.first.as<std::string>()] =
                    v.second.as<std::string>();
            }
        ...
        for (const auto& i : this->config.node)
        {
